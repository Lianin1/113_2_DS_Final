...
def process_data(sheet_id):
    ...
    try:
        ...
        sio.emit('status_update', {'status': 'Processing Google Sheet data...', 'progress': 20})
        if not run_script(['python', 'Auto.py', sheet_id]):
            sio.emit('error', 'Error occurred while processing Google Sheet data')
            return False
        
        # 2. 執行 HRanalyze.py
        sio.emit('status_update', {'status': 'Analyzing HR data...', 'progress': 40})
        if not run_script(['python', 'HRanalyze.py']):
            sio.emit('error', 'Error occurred while analyzing HR data')
            return False
        
        # 3. 並行執行報告生成和圖表生成
        def generate_reports():
            sio.emit('status_update', {'status': 'Generating analysis report...', 'progress': 60})
            run_script(['python', 'hr_analysis_feedback.py'])
        
        def generate_charts():
            # 執行所有圖表生成腳本
            sio.emit('status_update', {'status': 'Generating charts...', 'progress': 70})
            chart_scripts = [
                'boxplot_col.py',
                'histogram_col.py',
            ...
            # 最後執行圖表分析
            sio.emit('status_update', {'status': 'Generating chart analysis report...', 'progress': 80})
        ...
        sio.emit('status_update', {'status': 'All processing complete!', 'progress': 100})
...
def upload():
    ...
    if not sheet_id:
        return jsonify({'success': False, 'error': 'Sheet ID not provided'})
