...
df["性別"] = df["性別"].map({0: "男", 1: "女"})
df["部門"] = df["部門"].map({0: "人力資源", 1: "財務", 2: "技術", 3: "行銷", 4: "運營"})
df["Level"] = df["Level"].map({0: "初階", 1: "中階", 2: "高階"})
...
請將 (1)-(3) 整合為「人資分析與建議」，(4) 獨立成「問卷設計建議」，並使用繁體字說明，不可有任何表情符號。
"""
    return prompt
...
# df["性別"] = df["性別"].map({0: "男", 1: "女"})
# df["部門"] = df["部門"].map({0: "人力資源", 1: "財務", 2: "技術", 3: "行銷", 4: "運營"})
# df["職等"] = df["職等"].map({0: "初階", 1: "中階", 2: "高階"})
# risk_5_df = df[df["離職風險分級"] == 5]
...
# 請將 (1)-(3) 整合為「人資分析與建議」，(4) 獨立成「問卷設計建議」，並使用繁體字說明，不可有任何表情符號，不可使用NotoSansJP讀不出來的文字。
# """
#     return prompt
